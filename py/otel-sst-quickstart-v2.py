...
def _add_sample_data(i):
    ...
    #Modify the lookup files to match location data for the example data
    f = _get_lookup_path('otel_sample_cmdb.csv')

    input('***ACTION REQUIRED***\nYou must edit the lookup files to match hostnames to site information. \
See the splunk/lookups folder for examples and change the search macros sites-lookup-name and \
cmdb-lookup-name to mach the new lookup flenames. Press enter when complete:')
