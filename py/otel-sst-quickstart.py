...
#
# A companion setup script to automate the changes to Splunk Sustainability Toolkit to allow for OpenTelemetry support
...
def get_input_for_auth():
    '''Collects input for Splunk server authentication from the user in CLI. Returns a dict for splunk_auth().'''
    i = {
        "host": None,
        "port": None,
        "username": None,
    ...
    if i['port'] == '8000':
        print('INFO: Port 8000 is usually the Splunk Web port to acces the UI, and not the port used for the management API. To validate, check what port your Universal Forwarders are sending to.')
        i['port'] = input('Enter your Splunk management port (usually 8089): ')
    if i['port'] == None:
        i['port'] = '8089'  
...
def splunk_auth(auth_input):
    ...
    try:
        ...
        print (e)
        print('Error: Could not authenticate to Splunk with the provided information. Please check the input and retry.')
        raise
    return service

def create_macro(service, macro_name, definition):
    '''Adds a new search macro to Splunk, when provided an active splunk service session, macro name, and macro definition.'''
    try:
...
def rename_macro(service, macro_name_old, macro_name_new):
    '''Renames an existing search macro in Splunk, when provided an active splunk service session, and the old & new macro names.'''
    try:
...
def delete_macro(service, macro_name):
    '''Deletes an existing search macro in Splunk, when provided an active splunk service session and the macro name.'''
...
def create_saved_search(service, search_name, search_query):
    '''Adds a new saved search to Splunk, when provided an active splunk service session, search name, and SPL query.'''
    try:
...
def delete_saved_search(service, search_name):
    '''Deletes a  saved search in Splunk, when provided an active splunk service session, and search name.'''    
    try:
        service.get(f'search')
...
def rename_saved_search(service, search_name_old, search_name_new):
    '''Renames an existing saved search in Splunk, when provided an active splunk service session, and the old & new macro names'''
    try:
        service.get(f'search')
...
def check_app(service, app_name):
    ...
    if not app_installed:
        apps = s.apps
        print(f'App {app_name} is not installed. Please navigate to http(s)://[splunkhostname]/en-US/manager/search/appsremote?offset=0&count=20&order=relevance&query=sustainability and install Sustainability Toolkit for Splunk.')
...
answer = input('Do you already know the name of your electricitymaps zones? If not, we can show you the options here by saying no (y/n):')
